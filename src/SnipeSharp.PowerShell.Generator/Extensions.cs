...
namespace SnipeSharp.PowerShell.Generator
{
    internal static class Extensions
    {
        ...
        internal static bool TryGetInterface(this INamedTypeSymbol symbol, string name, out INamedTypeSymbol _interface)
        {
            var a = symbol.Interfaces.SingleOrDefault(a => a?.ToDisplayString().Split('<')[0] == name);
            if(null == a)
            {
                _interface = default!;
                return false;
